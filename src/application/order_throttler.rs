...
impl OrderThrottler {
    ...
    pub async fn run(&mut self) {
        info!("OrderThrottler started (limit: {} orders/min)", self.max_orders_per_minute);
    ...
    async fn process_queue(&mut self) {
        ...
        while !self.queued_orders.is_empty() && self.can_accept_order() {
            if let Some(order) = self.queued_orders.pop_front() { // FIFO: pop_front
                info!("OrderThrottler: Processing queued order {}. Remaining: {}", 
                      order.id, self.queued_orders.len());
                self.forward_order(order).await;
            }
        }
    }
    ...
    async fn forward_order(&mut self, order: Order) {
        ...
        if let Err(e) = self.throttled_order_tx.send(order.clone()).await {
            tracing::error!("OrderThrottler: Failed to forward order {}: {}", order.id, e);
        } else {
...
mod tests {
    ...
    async fn test_accepts_order_under_limit() {
        ...
        let mut throttler = OrderThrottler::with_window(order_rx, throttled_tx, 5, Duration::from_secs(1));
        ...
        for i in 1..=3 {
            order_tx.send(create_test_order(&format!("order-{}", i))).await.unwrap();
        }
    ...
    async fn test_queues_order_over_limit() {
        ...
        let mut throttler = OrderThrottler::with_window(order_rx, throttled_tx, 2, Duration::from_secs(1));
    ...
    async fn test_fifo_order_preservation() {
        ...
        for _ in 0..10 {
            if let Some(order) = tokio::time::timeout(
    ...
    async fn test_no_order_loss() {
        ...
        assert_eq!(received_count, 20, "All 20 orders should be received, none lost");
    }
