...
mod tests {
    ...
    async fn test_event_bus_publish() {
        ...
        bus.subscribe(Arc::new(CountingListener { count: Arc::clone(&count) })).await;
    ...
    async fn test_event_bus_multiple_listeners() {
        ...
        bus.subscribe(Arc::new(CountingListener { count: Arc::clone(&count1) })).await;
        bus.subscribe(Arc::new(CountingListener { count: Arc::clone(&count2) })).await;
