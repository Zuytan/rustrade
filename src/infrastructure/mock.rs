...
impl MockExecutionService {
    ...
    pub fn with_costs(portfolio: Arc<RwLock<Portfolio>>, slippage_pct: f64, commission_per_share: f64) -> Self {
        Self { 
            portfolio,
            orders: Arc::new(RwLock::new(Vec::new())),
            slippage_pct,
...
impl ExecutionService for MockExecutionService {
    async fn execute(&self, order: Order) -> Result<()> {
        ...
        let commission = Decimal::from_f64(self.commission_per_share).unwrap_or(Decimal::ZERO) * order.quantity;
        
        if self.slippage_pct > 0.0 || self.commission_per_share > 0.0 {
            info!("MockExecution: Order {} - Slippage: {:.4}%, Commission: ${:.4}, Total Cost Impact: ${:.4}",
                order.id, self.slippage_pct * 100.0, commission, 
        ...
        match order.side {
            crate::domain::types::OrderSide::Buy => {
                ...
                let pos = port.positions.entry(order.symbol.clone()).or_insert(crate::domain::portfolio::Position {
        ...
        info!("MockExecution: Order {} placed and executed on Exchange.", order.id);
        Ok(())
    }
