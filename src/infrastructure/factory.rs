        ...
        match config.mode {
            Mode::Mock => {
                ...
                let market_data: Arc<dyn MarketDataService> =
                    if config.platform.use_real_market_data {
                            ...
                            crate::config::AssetClass::Crypto => {
                                ...
                                Arc::new(
                                    BinanceMarketDataService::builder()
                                        .api_key(config.broker.binance.api_key.clone())
                                        .base_url(config.broker.binance.base_url.clone())
                                        .ws_url(config.broker.binance.ws_url.clone())
                                        .build(),
                                )
                            }
                            crate::config::AssetClass::Stock => {
                                Arc::new(
                                    AlpacaMarketDataService::builder()
                                        .api_key(config.broker.alpaca.api_key.clone())
                                        .api_secret(config.broker.alpaca.secret_key.clone())
                                        // Removed invalid .base_url call
                                        .data_base_url(config.broker.alpaca.data_url.clone())
                                        .api_base_url(config.broker.alpaca.base_url.clone())
            ...
            Mode::Alpaca => {
                let market_service = AlpacaMarketDataService::builder()
                    .api_key(config.broker.alpaca.api_key.clone())
                    .api_secret(config.broker.alpaca.secret_key.clone())
                    .ws_url(config.broker.alpaca.ws_url.clone())
                    .data_base_url(config.broker.alpaca.data_url.clone())
                    .api_base_url(config.broker.alpaca.base_url.clone())
                    .min_volume_threshold(
                        config
            ...
            Mode::Binance => {
                ...
                let execution_service = BinanceExecutionService::new(
                    config.broker.binance.api_key.clone(),
                    config.broker.binance.secret_key.clone(),
